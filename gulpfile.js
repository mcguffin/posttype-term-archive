...
const onPackage = function(bundle) {
	// extract from
	Object.keys(bundlemap).sort().forEach(src => {
		//  distinct
...
const config = {
	sass : {
		...
		sourceMap:true,
		functions: {
			'b64($str)': function(str) {
				var buffer = Buffer.from( str.getValue() );
				var out = new nodeSass.types.String(buffer.toString('base64'));
				return out;
			}
		},
...
/**
 *	wp i18n make-json lack textdomani prefix in themes
 */
// https://github.com/wp-cli/i18n-command/issues/197
...
gulp.task('i18n:strings-from-json', cb => {
	...
	strings = xt.parse_files( glob.sync('./json/*.json'), common_mapping, strings);
		...
		let tasks = glob.sync("./src/js/**/index.js")
			...
			.map( entry => {
				...
				return browserify({
					...
					//*
					.transform('uglifyify', { global: true, }) // 2x faster
					/*/
					.plugin('tinyify')
					//*/
...
gulp.task('i18n', gulp.series( 'i18n:strings-from-json','i18n:make-pot','i18n:fix-pot','i18n:make-json', 'i18n:theme-fix-json'));
